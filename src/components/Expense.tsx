...
export default function Expense(props: any) {
    ...
    const {expenseType, store, amount, tax, total, purchaseDate, id, header, totals, handleDelete, handleEdit} = props
    ...
    return (
        <div className={`expense ${header ? "expense-header" : ""} ${totals ? "expense-totals" : ""}`} onMouseOver={handleMouseOver} onMouseOut={handleMouseLeave}>
            ...
            <p>{total}</p>
            <p>{purchaseDate}</p>
            {!(header || totals) && mouseOver && <img className="expense--icon" src={pencilIcon} onClick={() => handleEdit(id)} />}
